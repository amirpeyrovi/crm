...
@AdminAnnotation
@CrossOrigin
@RequestMapping(value = "/v1/admin/role/{roleId}/permission", produces = "application/json")
public class AdminRolePermissionRestController implements RestControllerInterface {
    ...
    public Object findAll(@RequestParam(required = false, defaultValue = "0") String page,
            ...
            , @PathVariable("roleId") String roleIdString
    ) {
        ...
        AdminRole adminRole = this.adminRoleService.findById(roleId);
        ValidateObject validateObject = this.adminRolePermissionValidate.findAll();
        if (validateObject.getResult().equals("success")) {
            Page<AdminRolePermission> adminRolePermissions = this.adminRolePermissionService.findAllItemByAdminRole(adminRole, PageableRequest.getInstance().createPageRequest(page, "Product", sortProperty, sortOrder));
...
//        if (validateObject.getResult().equals("success")) {
//            adminRolePermission.setAdminPermission(this.adminPermissionService.findById(adminRolePermission.getAdminPermission().getPermissionId()));
//            adminRolePermission.setAdminRole(this.adminRoleService.findOne(adminRolePermission.getAdminRole()));
//            return new ApiResponse("Success", Arrays.asList(this.adminRolePermissionService.addNewItem(adminRolePermission).convert2Object()))
//                    .getSuccessResponse();
//        } else {
//            return new ApiResponse("Error", 102, validateObject.getMessages())
//                    .getFaultResponse();
...
//    }
    ...
    public Object updateOne(@PathVariable("roleId") String roleIdString
            , @RequestBody AdminRolePermissionAddDTO adminRolePermissionAddDTO) {
        ...
        Long roleId = Long.valueOf(roleIdString);
        AdminRolePermission adminRolePermission = adminRolePermissionAddDTO.convert2Object();
        adminRolePermission.setAdminRole(this.adminRoleService.findById(roleId));
        adminRolePermission.setAdminPermission(this.adminPermissionService.findById(adminRolePermission.getAdminPermission().getPermissionId()));

        ValidateObject validateObject = this.adminRolePermissionValidate.validateUpdateItem(adminRolePermission);
        if (validateObject.getResult().equals("success")) {
            try {
                return new ApiResponse("Success", Arrays.asList(this.adminRolePermissionService.updateItem(adminRolePermission).convert2Object()))
                        .getSuccessResponse();
